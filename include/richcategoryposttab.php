...
if ( ! class_exists( 'richcategoryposttabWidget' ) ) { 
	class richcategoryposttabWidget extends richcategoryposttabLib {
		...
		*/ 
		public function init() {
			...
			add_action( 'wp_ajax_rcpt_getTotalPosts',array( &$this, 'rcpt_getTotalPosts' ) );
			add_action( 'wp_ajax_rcpt_getPosts',array( &$this, 'rcpt_getPosts' ) ); 
			add_action( 'wp_ajax_rcpt_getMorePosts',array( &$this, 'rcpt_getMorePosts' ) );
			
			add_action( 'wp_ajax_nopriv_rcpt_getTotalPosts', array( &$this, 'rcpt_getTotalPosts' ) );
			add_action( 'wp_ajax_nopriv_rcpt_getPosts', array( &$this, 'rcpt_getPosts' ) ); 
			add_action( 'wp_ajax_nopriv_rcpt_getMorePosts', array( &$this, 'rcpt_getMorePosts' ) ); 
			
			add_shortcode( 'richcategoryposttab', array( &$this, 'richcategoryposttab' ) ); 
		...
		*/  
		public function rcpt_getTotalPosts( $category_id, $post_search_text, $c_flg, $is_default_category_with_hidden ) { 
		...
		*/
		public function richcategoryposttab( $params = array() ) { 	
			
			if(isset($params["id"]) && trim($params["id"]) != "" && intval($params["id"]) > 0) {
				$richcategoryposttab_id = $params["id"]; 
				$rcpt_shortcode = get_post_meta( $richcategoryposttab_id ); 
				
				foreach ( $rcpt_shortcode as $sc_key => $sc_val ) {			
					$rcpt_shortcode[$sc_key] = $sc_val[0];			
				} 
				
				if(!isset($rcpt_shortcode["number_of_post_display"]))	
					$rcpt_shortcode["number_of_post_display"] = 0;
				if(!isset($rcpt_shortcode["category_id"]))	
					$rcpt_shortcode["category_id"] = 0;
					
				$this->_config = shortcode_atts( $this->_config, $rcpt_shortcode ); 
		...
		*/	
		public function rcpt_getMorePosts() {
			...
			*/
			...
			$_limit_end = ( isset( $_REQUEST["number_of_post_display"])?esc_attr( $_REQUEST["number_of_post_display"] ):rcpt_number_of_post_display ); 
			...
			*/	
			...
			require( $this->getrichcategoryposttabTemplate( 'fronted/ajax_load_more_posts.php' ) );	
		...
		*/
		public function rcpt_getPosts() {
		   ...
		   require( $this->getrichcategoryposttabTemplate( 'fronted/ajax_load_posts.php' ) );	
