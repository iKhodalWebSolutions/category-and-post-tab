...
function rcpt_init_elements(){
	jQuery(document).ready(function($){
		$("#richcategoryposttab .wea_content .pn-active").each(function(){  
			rcpt_ikh_resize_elements($(this).attr("id")); 
		});
	});
} 

function rcpt_reload_current_elements(ipc_crid){
	jQuery(document).ready(function($){ 
		rcpt_ikh_resize_elements(ipc_crid); 
...
function rcpt_ikh_resize_elements(elementId) {
	jQuery(document).ready(function($){
			...
			if(ikh_config_elements["ikh_templates"]=="fit_to_sides") {
				rcpt_ikh_fit_to_sides(ikh_config_elements,root_element);
			} else if(ikh_config_elements["ikh_templates"]=="fixed_width_left") {
				rcpt_ikh_fixed_width_left(ikh_config_elements,root_element);
			} else if(ikh_config_elements["ikh_templates"]=="fixed_width_center") {
				rcpt_ikh_fixed_width_center(ikh_config_elements,root_element);
			}
			
			$(root_element).find(".ik-post-load-more").show();
			$(root_element).find(".rcpt-simple-paging").show(); 
	});	
}

function rcpt_ikh_fit_to_sides(ikh_config_elements,root_element) {
	...
	jQuery(document).ready(function($){
		...
		if(ikh_main_width<=ikh_item_area_width) {
				...
				$(ikh_element).find(".rcpt-simple-paging").removeAttr("style").css("position","relative");	 
		} else {
				...
				for( var ikhw = 60; ikhw > 0; ikhw-- ) {
					...
					if( ikh_main_width >= ikh_items_width ) {
						...
						for ( var ikh_j1=0; ikh_j1 < $(ikh_element).find(".ikh-post-item-box").length; ikh_j1++ ) {
							...
							var ikh_mrg_lft = 0; 
							if( ikh_j1 == 0 ) { 
								rcpt_ikh_set_element_postion(ikh_element,ikh_j1,ikh_first_left,0,ikh_config_elements);
								ikh_mrg_lft = 0;
							} else {
								if( (ikh_j1%ikhw) == 0 ) {
									ikh_last_left = ikh_first_left;
								...
								if( ((ikh_j1-1)%(ikhw)) == 0 && ikhw>1 ) 
									ikh_last_left = ikh_last_left - ikh_margin_left-ikh_border_difference;
								rcpt_ikh_set_element_postion(ikh_element,ikh_j1,ikh_last_left,(ikh_num_lines*(ikh_image_height1+ikh_margin_bottom)),ikh_config_elements);	
		...
		rcpt_ikh_common_innerelement_load(ikh_element);  
	
	});
}	

function rcpt_ikh_fixed_width_left(ikh_config_elements,root_element) {
...
function rcpt_ikh_fixed_width_center(ikh_config_elements, root_element) {
	...
	jQuery(document).ready(function($){
		...
		for ( var ikh_j1=0; ikh_j1 < $(ikh_element).find(".ikh-post-item-box").length; ikh_j1++ ) {
			...
			var ikh_mrg_lft = 0; 
			if( ikh_j1 == 0 ) { 
				rcpt_ikh_set_element_postion(ikh_element,ikh_j1,(ikh_first_left+ikh_remaining_width_left+((ikh_org_margin_left/2)/2)),0,ikh_config_elements);
				ikh_mrg_lft = 0;
				ikh_last_left = ikh_first_left+ikh_remaining_width_left+((ikh_org_margin_left/2)/2);
			} else {
				if( (ikh_j1%ikhw) == 0 ) {
...
function rcpt_ikh_common_innerelement_load(ikh_element){
	jQuery(document).ready(function($){
		...
		} if($(ikh_element).find(" .rcpt-simple-paging").length>0){
			main_final_height = main_final_height + ($(ikh_element).find(".rcpt-simple-paging").height()+30);
			var left_bottom_pos = ($(ikh_element).find(" .rcpt-simple-paging").width()/2); 
			$(ikh_element).find(" .rcpt-simple-paging").css("top",pagination_top+"px").css("position","absolute");
			$(ikh_element).find(" .rcpt-simple-paging").css("left",(($(ikh_element).width()/2)-left_bottom_pos)+"px");
			$(ikh_element).css("display","block"); 
			$(ikh_element).find(" .item-posts-wrap").css("height",main_final_height+"px");
			$(ikh_element).css("height",main_final_height+"px");
		} else {
		...
		var ikh_main_width = parseInt($(ikh_element).width());      
		if(ikh_main_width<=ikh_item_area_width) {
			$(ikh_element).find(" .rcpt-simple-paging").css("position","relative").removeAttr("style").show();
			$(ikh_element).find(" .ik-post-load-more").css("position","relative").removeAttr("style").show();	
		    $(ikh_element).find(" .rcpt-simple-paging").css("position","relative");
...
function rcpt_ikh_set_element_postion(ikh_element,ikh_ele,ikh_left,ikh_top,ikh_config_elements_1){			 
...
function rcpt_loadMorePosts(category_id,limit,elementId,total,request_obj){
	if(flg_v1==1) return;
	jQuery(document).ready(function($){ 
			...
			request_obj.action = 'rcpt_getMorePosts';
			request_obj.security = richcategoryposttab.rcpt_security;
			...
			$.ajax({
				url: richcategoryposttab.rcpt_ajax_url, 
				data: request_obj,
				success:function(data) {     
					rcpt_printData(elementId,data,"loadmore");
				},error: function(errorThrown){ console.log(errorThrown);}
			});
	});
}
function rcpt_fillPosts(elementId,category_id,request_obj,flg_pr){
	if(flg_v1==1) return;
 	jQuery(document).ready(function($){
			...
			if($("#"+elementId).hasClass('pn-active') && flg_pr==1) {			  
				...
				return;
			} else if( $("#"+elementId).parent().parent().parent().hasClass("rcpt_allow_autoclose_accordion")  && flg_pr==1 ) {  
			...
			request_obj.action = 'rcpt_getPosts';
			request_obj.security = richcategoryposttab.rcpt_security;
			request_obj.category_id = category_id;
			request_obj.post_search_text = post_search_text; 
			//security: richcategoryposttab.rcpt_security,
		 	$.ajax({
				url: richcategoryposttab.rcpt_ajax_url,				
				data: request_obj,
				success:function(data) { 
					rcpt_printData(elementId,data,"fillpost"); 
				},error: function(errorThrown){ console.log(errorThrown);}
			});   
	});	 
}
function rcpt_printData(elementId,data,flg){
	jQuery(document).ready(function($){
		...
		if(flg=="loadmore"){  
			...
			if ($("#"+elementId).parent().parent().hasClass("simple_numeric_pagination") || $("#"+elementId).parent().parent().hasClass("next_and_previous_links")) {			
				...
				$(root_element).find(".item-posts").find(".rcpt-simple-paging").remove();
		...
		rcpt_reload_current_elements(elementId);
		flg_v1 = 0;	
	});	  
	
}
var flg_ms_hover = 0;
function rcpt_pr_item_image_mousehover(ob_pii){ 
...
function rcpt_pr_item_image_mouseout(ob_pii){
	jQuery(document).ready(function($){ 
...
function rcpt_cat_tab_ms_out(ob_ms_eff){
	jQuery(document).ready(function($){ 
		$(ob_ms_eff).removeClass("pn-active-bg"); 	
	});
}
function rcpt_cat_tab_ms_hover(ob_ms_eff){
	jQuery(document).ready(function($){ 
		$(ob_ms_eff).addClass("pn-active-bg"); 	
	});
} 

function rcpt_init_accordion(){
	jQuery(document).ready(function($){
		...
		setInterval(function(){  
			 $("#richcategoryposttab .wea_content .pn-active").each(function(){ 
				...
				$(ikh_element).find(".ikh-post-item").each(function(){
					 if( $(this).height() <= 50 ){
						rcpt_ikh_resize_elements(attr_id);  
					 }
				});  
			});
		},500);
...
if ( window.addEventListener ) { 
	window.addEventListener( "load", rcpt_init_accordion, false );
}
else 
{    
	if ( window.attachEvent ) { 
		  window.attachEvent( "onload", rcpt_init_accordion );
	} else {
		 if ( window.onLoad ) {
		   window.onload = rcpt_init_accordion;
		 }
	}	 
}